...
#include <math.h>
#include <time.h>
/*** Start of inlined file: sqpcheader.h ***/
//#define _DEBUG_DUMP
...
/*** Start of inlined file: sqobject.h ***/

/*** Start of inlined file: squtils.h ***/
...
//----------------------------------------
static bool _init_add_systemfunction(HSQUIRRELVM v,const SQChar* methodName,SQFUNCTION methodFunction,SQInteger nparamscheck,const SQChar* typemask);
static SQInteger _ptr_string_format(HSQUIRRELVM v);
...
/*** End of inlined file: sqobject.h ***/



/*** Start of inlined file: sqstate.h ***/
...
/*** End of inlined file: sqstate.h ***/



/*** End of inlined file: sqpcheader.h ***/



/*** Start of inlined file: sqvm.h ***/

/*** Start of inlined file: sqopcodes.h ***/
...
enum SQOpcode
{
	...
	_OP_CLOSE=              0x3C,
	//---------------------------
	_OP_PTR_GET=             0x3D,  //Add By Lig
	_OP_PTR_SET=             0x3E,  //Add By Lig, 参数0为target，参数1为指针，参数2为指针偏移值，参数3为指针类别
	_OP_PTR_CALL=            0x3F,  //Add By Lig
};
...
/*** End of inlined file: sqopcodes.h ***/
...
////////////////////////////////////////////////////////////////////////////
	...
	SQObjectPtr _nativeobjectstable; //Add by Lig
	...
	//-------------------
	SQBool _vm_fastcall_flag; //add by lig,快速C/Cpp函数调用时的不可重入标志
};
...
/*** End of inlined file: sqvm.h ***/


/*** Start of inlined file: sqstring.h ***/
#ifndef _SQSTRING_H_
#define _SQSTRING_H_
...
/*** End of inlined file: sqstring.h ***/


/*** Start of inlined file: sqtable.h ***/
/*
...
/*** End of inlined file: sqtable.h ***/


/*** Start of inlined file: sqarray.h ***/
...
/*** End of inlined file: sqarray.h ***/


/*** Start of inlined file: sqfuncproto.h ***/
enum SQOuterType {
	otLOCAL = 0,
	otOUTER = 1
};
...
/*** End of inlined file: sqfuncproto.h ***/


/*** Start of inlined file: sqclosure.h ***/
...
/*** End of inlined file: sqclosure.h ***/


/*** Start of inlined file: squserdata.h ***/
...
/*** End of inlined file: squserdata.h ***/


/*** Start of inlined file: sqcompiler.h ***/
struct SQVM;
...
#define TK_CONST 324
//--------------------
#define TK_POINTER 325

typedef void(*CompilerErrorFunc)(void *ud, const SQChar *s);
bool Compile(SQVM *vm, SQLEXREADFUNC rg, SQUserPointer up, const SQChar *sourcename, SQObjectPtr &out, bool raiseerror, bool lineinfo);


/*** End of inlined file: sqcompiler.h ***/


/*** Start of inlined file: sqfuncstate.h ***/
///////////////////////////////////
...
/*** End of inlined file: sqfuncstate.h ***/


/*** Start of inlined file: sqclass.h ***/
struct SQInstance;
...
struct SqNativeClass : public CHAINABLE_OBJ
{
	SQUserPointer _typetag;
	SQRELEASEHOOK _hook;
	bool _locked;
	SQInteger _constructoridx;
	SQInteger _udsize;
	SQUserPointer _pNativeClassInstance;
	SQTable _members;
...
/*** End of inlined file: sqclass.h ***/


/*** Start of inlined file: sqlexer.h ***/
...
void sq_pushnativeobjectstable(HSQUIRRELVM v)
{
	v->Push(v->_nativeobjectstable);
}
...
void SQLexer::Init(SQSharedState *ss, SQLEXREADFUNC rg, SQUserPointer up,CompilerErrorFunc efunc,void *ed)
{
	...
	//------------------------------
	ADD_KEYWORD(var,TK_LOCAL); //Lig Add
	_readf = rg;
	_up = up;
...
SQInteger SQLexer::Lex()
{
	...
	while(CUR_CHAR != SQUIRREL_EOB) {
		...
		case _SC('\n'):
			...
			_currentcolumn=1;
			continue;
		case _SC('#'): //Edit By Lig
			{
				SQInteger ret = CUR_CHAR;
				NEXT();RETURN_TOKEN(ret);continue;
				//if (scisalpha(CUR_CHAR) || CUR_CHAR == _SC('_'))
				//{
				//	SQInteger t = ReadID();
				//	if(t == TK_IDENTIFIER)
				//	RETURN_TOKEN(TK_IMM_IDENTIFIER);
				//}
				//else {
				//	LexLineComment();
				//	continue;
		...
		case _SC('-'):
			...
			else if  (CUR_CHAR == _SC('>')){ NEXT(); RETURN_TOKEN(TK_POINTER);}
			else RETURN_TOKEN('-');
		case _SC('+'):
			NEXT();
...
bool SQTable::Set(const SQObjectPtr &key, const SQObjectPtr &val)
{
	...
	return NewSlot(key, val); //Add By Lig,֧3avaScriptһ7(
	//return false;
}
...
bool SQVM::ARITH_OP(SQUnsignedInteger op,SQObjectPtr &trg,const SQObjectPtr &o1,const SQObjectPtr &o2)
{
	...
	switch(tmask) {
		...
		case (OT_FLOAT):{
			...
			trg = res; }
			break;
		case (OT_USERPOINTER|OT_INTEGER): //Add By Lig ,for extend
			{
				unsigned char* p1 =(unsigned char*) _userpointer(o1);
...
bool SQVM::Init(SQVM *friendvm, SQInteger stacksize)
{
	...
	_stackbase = 0;
	_top = 0;
	_vm_fastcall_flag = SQFalse; //Add by lig
	if(!friendvm) {
		_roottable = SQTable::Create(_ss(this), 0);
		_nativeobjectstable = SQTable::Create(_ss(this), 0); //Add by lig
		sq_base_register(this);
	}
	else {
		_roottable = friendvm->_roottable;
		_nativeobjectstable = friendvm->_nativeobjectstable; //Add by lig
	...
	//
	{
		for(;;)
		{
			...
			case _OP_CALL: {
					...
					default:
						Raise_Error(_SC("attempt to call '%s',stack %d:%d"), GetTypeName(clo),_stackbase,arg1);
						SQ_THROW();
					}
				}
				  continue;
			...
			//----------------------------------------
			case _OP_PTR_GET:{
					if(STK(arg1)._type!=OT_USERPOINTER)
					{
						Raise_Error(_SC("only USERPOINTER can be used as a native pointer!")); 
						SQ_THROW();
					}
					SQInteger idx = 0;
					if(arg2!=0 && STK(arg2)._type!=OT_INTEGER)
					{
						Raise_Error(_SC("only integer can be used as index of native pointer!"));
						SQ_THROW();
					}
					else if(arg2>0) idx=STK(arg2)._unVal.nInteger; //如果arg2为0,表示没有参数写入,idx为0

					switch(arg3)
					{
					case 0:
						TARGET = (SQInteger) (*((unsigned char*) ((unsigned char*)(STK(arg1)._unVal.pUserPointer) + idx)));
						break;
					case 1:
						TARGET = (SQInteger) (*((unsigned short*) ((unsigned char*)(STK(arg1)._unVal.pUserPointer) + idx)));
						break;
					case 2:
						TARGET = (SQInteger) ((*((unsigned int*) ((unsigned char*)((unsigned char*)(STK(arg1)._unVal.pUserPointer) + idx)) )) & 0xFFFFFF);
						break;
					case 3:
						TARGET = (SQInteger) (*((unsigned int*) ((unsigned char*)(STK(arg1)._unVal.pUserPointer) + idx)));
						break;
					case 4:
						{
					...
					case 6:
						{
							unsigned int target = (unsigned int)(*((unsigned int*)( (unsigned char*)(STK(arg1)._unVal.pUserPointer) +idx)));
							TARGET = (SQInteger)((((unsigned int)(target) & 0xff000000) >> 24) | (((unsigned int)(target) & 0x00ff0000) >> 8) | (((unsigned int)(target) & 0x0000ff00) << 8) | (((unsigned int)(target) & 0x000000ff) << 24));
							break;
						}
					case 7:
						TARGET = (SQFloat) (*((float*)( (unsigned char*)(STK(arg1)._unVal.pUserPointer) + idx)));
						break;
					case 8:
						TARGET = (SQFloat) (*((double*) ((unsigned char*)(STK(arg1)._unVal.pUserPointer) + idx)));
						break;
					case 9:
						TARGET = (SQInteger) (*((int*) ((unsigned char*)(STK(arg1)._unVal.pUserPointer) + idx)));
						break;
					default:
						TARGET = (SQInteger) (*((unsigned char*) ((unsigned char*)(STK(arg1)._unVal.pUserPointer) + idx)));
					}
			...
			case _OP_PTR_SET:{
					...
					else if (arg2 == 0) //如果idx参数的位置是0，那么实际值也是0，而不是堆栈0位置的值
						idx = 0;
					else 
						idx = STK(arg2)._unVal.nInteger; //堆栈中该位置的值
					if(TARGET._type!=OT_USERPOINTER)
					{
						Raise_Error(_SC("only USERPOINTER can be used as a native pointer!")); 
						SQ_THROW();
					}
					SQObject ptr = TARGET;
					SQInteger val = STK(arg1)._unVal.nInteger;
			
					switch (arg3)
					{
					case 0:
						*((unsigned char*)((unsigned char*)(ptr._unVal.pUserPointer) + idx)) = (unsigned char)val;
						break;
					case 1: 
						memcpy((unsigned char*)(ptr._unVal.pUserPointer) + idx,&(STK(arg1)._unVal.nInteger),2);
						break;
					case 2:
						memcpy((unsigned char*)(ptr._unVal.pUserPointer) + idx,&(STK(arg1)._unVal.nInteger),3);
						break;
					case 3:
						*((unsigned int*)((unsigned char*)(ptr._unVal.pUserPointer) + idx)) = (unsigned int)val;
						break;
					case 4:
					{
						*((unsigned char*)((unsigned char*)(ptr._unVal.pUserPointer) + idx)) = ((val>>8) & 0xFF);
						*((unsigned char*)((unsigned char*)(ptr._unVal.pUserPointer) + idx + 1)) = (val & 0xFF);
						break;
					}
					case 5:
					{
						*((unsigned char*)((unsigned char*)(ptr._unVal.pUserPointer)+ idx)) = ((val>>16) & 0xFF);
						*((unsigned char*)((unsigned char*)(ptr._unVal.pUserPointer)+idx+1)) = ((val>>8) & 0xFF);
						*((unsigned char*)((unsigned char*)(ptr._unVal.pUserPointer)+idx+2)) = (val & 0xFF);
						break;
					}
					case 6:
					{
						*((SQInteger*)((unsigned char*)(ptr._unVal.pUserPointer)+idx)) = (SQInteger)((((unsigned int)(val) & 0xff000000) >> 24) | (((unsigned int)(val) & 0x00ff0000) >> 8) | (((unsigned int)(val) & 0x0000ff00) << 8) | (((unsigned int)(val) & 0x000000ff) << 24));
						break;
					}
					case 7:
						*((SQFloat*)((unsigned char*)(ptr._unVal.pUserPointer)+idx))  = STK(arg1)._unVal.fFloat;
						break;
					case 8:
						*((double*)((unsigned char*)(ptr._unVal.pUserPointer)+idx))  = (double)(STK(arg1)._unVal.fFloat);
						break;
					case 9:
						*((SQInteger*)((unsigned char*)(ptr._unVal.pUserPointer)+idx))  = val;
						break;
					default:
						*((unsigned char*)((unsigned char*)(ptr._unVal.pUserPointer) + idx)) = (unsigned char)val;
					}
				...
				continue;
			case _OP_PTR_CALL: { 
					if(_vm_fastcall_flag==SQTrue)
					{
						Raise_Error(_SC("C/C++ function re_enter vm!")); 
						return false;
					}
					_vm_fastcall_flag=SQTrue;//设定不可重入
					SQNativeClosure* nclosure=(SQNativeClosure*)arg1;
					SQInteger nargs=arg3;
					SQInteger newbase=_stackbase + arg2;
					SQInteger nparamscheck = nclosure->_nparamscheck;
					SQInteger newtop = newbase + nargs + nclosure->_noutervalues;
					SQInteger _pre_stackbase=_stackbase;
					SQInteger _pre_top=_top;

					if(nparamscheck && (((nparamscheck > 0) && (nparamscheck != nargs)) ||
						((nparamscheck < 0) && (nargs < (-nparamscheck)))))
					{
						Raise_Error(_SC("wrong number of parameters"));
						_vm_fastcall_flag=SQFalse;//设定可重入
						return false;
					}

					if(nparamscheck>0) //允许参数判定
					{
						SQInteger tcs;
					...
					nparamscheck = _top;  //如果有返回值，这里记录返回值的位置
					//-------------------
					if (ret < 0)
					{
						_stackbase = _pre_stackbase;
						_top = _pre_top;
						while (newtop >= _top) 
							_stack._vals[newtop--].Null();
					...
					{
						if(ret)
							STK(arg0) = _stack._vals[nparamscheck-1];
						else 
							STK(arg0).Null();
					}
					//_printcallstack(this);
					while (newtop >= _top) 
						_stack._vals[newtop--].Null();
					_vm_fastcall_flag=SQFalse;//设定可重入
				}
...
#define LOCAL  4
#define OUTER  5
#define PTR_ZERO_IDX  16  //add by lig
#define PTR_OFFSET_IDX  32  //add by lig
struct SQExpState {
...
public:
			...
			if(_token == TK_CONSTRUCTOR && tok == TK_IDENTIFIER) {
				...
				if(tok > 255) {
					...
					case TK_FLOAT:
						etypename = _SC("FLOAT");
						break;
					//case TK_IMM_IDENTIFIER:
					//	etypename = _SC("IMM_IDENTIFIER");
					//	break;
					default:
	...
	void Expression()
	{
		...
		case TK_MODEQ:{
			...
			case _SC('='): //ASSIGN
				...
				default:
					...
					if(ds>=PTR_OFFSET_IDX) //add by lig
					{
						SQInteger src = _fs->PopTarget();
						SQInteger idx = _fs->PopTarget();
						SQInteger dst = _es.epos;
						_fs->AddInstruction(_OP_PTR_SET, dst, src, idx, ds - PTR_OFFSET_IDX);
					}
					else if (ds >= PTR_ZERO_IDX)
					{
						SQInteger src = _fs->PopTarget();
						SQInteger dst = _es.epos;
						_fs->AddInstruction(_OP_PTR_SET, dst, src, 0, ds - PTR_ZERO_IDX);
					}
					break;
				}
				break;
		...
		case _SC('?'): {
			...
			break;
		case TK_POINTER : { //add by Lig
			Lex();
			SQInteger ptr=_fs->PopTarget();
			SQObjectPtr ModName;
			SQObjectPtr FuncName;
			if(_token==TK_IDENTIFIER)
				ModName=_fs->CreateString(_lex._svalue);
			else
				Error(_SC("invalid native pointer call!"));
			Lex();
			if(_token==_SC(':'))
			{
				Lex();
				FuncName=Expect(TK_IDENTIFIER);
			}
			else
			{
				FuncName=ModName;
				ModName=SQString::Create(_ss(_vm),_SC("_sys_ptr_lib"), -1);
			}
			if(_token!=_SC('('))
				Error(_SC("invalid native pointer call!"));
			
			SQObjectPtr object_table;
			SQObjectPtr object_function;
			if(!_vm->_nativeobjectstable._unVal.pTable->Get(ModName,object_table))
				Error(_SC("invalid native pointer object!"));
			if(!object_table._unVal.pTable->Get(FuncName,object_function))
				Error(_SC("invalid native pointer method!"));
			Lex();
			_fs->AddInstruction(_OP_MOVE,_fs->PushTarget(),ptr);
			Direct_FunctionCallArgs((SQUnsignedInteger)object_function._unVal.pNativeClosure);
			}
			break;
		}
		_es = es;
			...
			case _SC('@'): {
					...
					if (_token == _SC('['))
					{
					   ...
					   if (_token == _SC(']'))
					   {
						   ...
						   {
							   SQInteger idx = _fs->PopTarget();
							   SQInteger ptr_idx = _fs->PopTarget();
							   _fs->AddInstruction(_OP_PTR_GET, _fs->PushTarget(), ptr_idx, idx, 0);
						   }
						   else
						   {
							   _es.etype = PTR_OFFSET_IDX; //改变类型，等待赋值后加入指令
						   }
						   break;
					   }
					   else Error(_SC("@(data type)[] expression expected...."));
					}
					else if (_token == TK_IDENTIFIER)
					{
					   const SQChar* methodName = _lex._svalue;
					   int type = 0;
					   if (scstrcmp(methodName, _SC("u8")) == 0) type = 0;
					   else if (scstrcmp(methodName, _SC("char")) == 0) type = 0;
					   else if (scstrcmp(methodName, _SC("u16")) == 0) type = 1;
					   else if (scstrcmp(methodName, _SC("u24")) == 0) type = 2;
					   else if (scstrcmp(methodName, _SC("u32")) == 0) type = 3;
					   else if (scstrcmp(methodName, _SC("bu16")) == 0) type = 4;
					   else if (scstrcmp(methodName, _SC("bu24")) == 0) type = 5;
					   else if (scstrcmp(methodName, _SC("bu32")) == 0) type = 6;
					   else if (scstrcmp(methodName, _SC("float")) == 0) type = 7;
					   else if (scstrcmp(methodName, _SC("double")) == 0) type = 8;
					   else if (scstrcmp(methodName, _SC("int")) == 0) type = 9;
					   else if (scstrcmp(methodName, _SC("short")) == 0) type = 10;
					   else Error(_SC("@(data type)[] expression expected...."));
					   Lex();
					   if (_token == _SC('['))
					   {
						   ...
						   if (_token == _SC(']'))
						   {
							   ...
							   if (_token != _SC('='))
							   {
								   SQInteger idx = _fs->PopTarget();
								   SQInteger ptr_idx = _fs->PopTarget();
								   _fs->AddInstruction(_OP_PTR_GET, _fs->PushTarget(), ptr_idx, idx, type);
							   }
							   else
							   {
								   _es.epos = pos;
								   _es.etype = PTR_OFFSET_IDX + type; //改变类型，等待赋值后加入指令
							   }
							   break;
						   }
						   else Error(_SC("@(data type)[] expression expected...."));
					   ...
					   else
					   {
						   ...
						   {
							   SQInteger ptr_idx = _fs->PopTarget();
							   _fs->AddInstruction(_OP_PTR_GET, _fs->PushTarget(), ptr_idx, 0, type);
						   }
						   else
						   {
							   _es.epos = pos;
							   _es.etype = PTR_ZERO_IDX + type; //改变类型，等待赋值后加入指令
						   }
						   break;
					   }
					}
	...
	void Direct_FunctionCallArgs(SQUnsignedInteger IntFunction) //Add by lig
	{
		SQInteger nargs = 1;//this
		 while(_token != _SC(')')) {
			 Expression();
		 ...
		 Lex();
		 for(SQInteger i = 0; i < (nargs - 1); i++) _fs->PopTarget();
		 //SQInteger stackbase = _fs->TopTarget();
		 SQInteger stackbase = _fs->PopTarget();
		 //SQInteger closure = _fs->PopTarget();
		 _fs->AddInstruction(_OP_PTR_CALL, _fs->PushTarget(),IntFunction , stackbase, nargs);
	}
	...
	void LocalDeclStatement()
	{
		...
		do {
			if(_token==TK_IDENTIFIER)
			{
				varname = _fs->CreateString(_lex._svalue);
				Lex();
				if(_token == _SC('=')) {
					Lex(); Expression();
					SQInteger src = _fs->PopTarget();
			...
			{
				Error(_SC("expected '%s'"), "IDENTIFIER ");
				break;
			}
		} while(1);
	}
...
//---------------------------
static SQInteger math_malloc(HSQUIRRELVM v)
{
    SQInteger n;
    sq_getinteger(v,2,&n);
	SQUserPointer ptr=sq_vm_malloc(n);
	if(ptr!=NULL)
	{
...
static SQInteger math_free(HSQUIRRELVM v)
{
	SQUserPointer ptr;
...
//---------------------------------
void register_nativeobjects(HSQUIRRELVM v,const char* moduleName,const char* methodName,SQFUNCTION methodFunction,SQInteger nparamscheck,const SQChar* typemask)
{
	int n=sq_gettop(v);
re_enter:
	sq_pushnativeobjectstable(v);
	sq_pushstring(v,moduleName,-1);
	if(SQ_SUCCEEDED(sq_get(v,-2))) //找到需要页面的执行函数
	{
		SQObjectType tp;
		if((tp=sq_gettype(v,-1))==OT_TABLE)
		{
			sq_pushstring(v,methodName,-1);
			sq_newclosure(v,methodFunction,0);
			sq_setnativeclosurename(v,-1,methodName);
...
SQRESULT sq_get_nativeobjects_function(HSQUIRRELVM v,const SQChar* moduleName,const SQChar* methodName)
{
	int n=sq_gettop(v);
re_enter:
	sq_pushnativeobjectstable(v);
	sq_pushstring(v,moduleName,-1);
	if(SQ_SUCCEEDED(sq_get(v,-2))) //找到需要对象名称
	{
		SQObjectType tp;
		if((tp=sq_gettype(v,-1))==OT_TABLE)
		{
			sq_pushstring(v,methodName,-1);
			if(SQ_SUCCEEDED(sq_get(v,-2))) //找到需要的执行函数
			{
				if((tp=sq_gettype(v,-1))==OT_NATIVECLOSURE) return SQ_OK;
			}
			else sq_settop(v,n);
		}
		else sq_settop(v,n);
...
static SQInteger _printcallstack(HSQUIRRELVM v)
{
	const char *name="";
	for(int i=v->_top;i>=0;i--)
	{
		printf("%03d",i);
		switch(v->_stack[i]._type)
        {
               case OT_NULL:
                   printf(_SC("[%s] NULL\n"),name);
                   break;
               case OT_INTEGER:
				   printf(_SC("[%s] %d\n"),name,v->_stack[i]._unVal.nInteger);
                   break;
               case OT_FLOAT:
              	   printf(_SC("[%s] %.14g\n"),name,v->_stack[i]._unVal.fFloat);
                   break;
               case OT_USERPOINTER:
                   printf(_SC("[%s] USERPOINTER 0x%08X\n"),name,v->_stack[i]._unVal.nInteger);
                   break;
               case OT_STRING:
                   printf(_SC("[%s] \"%s\"\n"),name,_stringval(v->_stack[i]));
                   break;
               case OT_TABLE:
                   printf(_SC("[%s] TABLE\n"),name);
                   break;
               case OT_ARRAY:
                   ...
                   break;
               case OT_CLOSURE:
                   printf(_SC("[%s] CLOSURE\n"),name);
                   break;
               case OT_NATIVECLOSURE:
                   printf(_SC("[%s] NATIVECLOSURE\n"),name);
                   break;
               case OT_GENERATOR:
                   printf(_SC("[%s] GENERATOR\n"),name);
                   break;
               case OT_USERDATA:
                   printf(_SC("[%s] USERDATA\n"),name);
                   break;
               case OT_THREAD:
                   printf(_SC("[%s] THREAD\n"),name);
                   break;
               case OT_CLASS:
                   ...
                   break;
               case OT_INSTANCE:
                   printf(_SC("[%s] INSTANCE\n"),name);
                   break;
               case OT_WEAKREF:
                   printf(_SC("[%s] WEAKREF\n"),name);
                   break;
               case OT_BOOL:{
                   SQBool bval;
                   sq_getbool(v,-1,&bval);
                   printf(_SC("[%s] %s\n"),name,v->_stack[i]._unVal.nInteger == SQTrue ? _SC("true"):_SC("false"));
                   }
                   break;
               default: assert(0); break;
        }
...
void _init_addtion_sysfunction(HSQUIRRELVM v)
{
	sq_pushroottable(v);
	sq_pushstring(v,"_printcallstack",-1);
	sq_newclosure(v,_printcallstack,0);
	sq_setnativeclosurename(v,-1,"_printcallstack");
	sq_newslot(v,-3,SQFalse);
	register_nativeobjects(v,_SC("_sys_ptr_lib"),_SC("format"),_ptr_string_format,0,NULL);
		
};

SQRESULT _ptr_format(HSQUIRRELVM v,SQInteger nformatstringidx,SQInteger *outlen,SQChar *dest) //add by lig
{
    const SQChar *format;
     SQChar fmt[MAX_FORMAT_LEN];
    sq_getstring(v,nformatstringidx,&format);
    SQInteger format_size = sq_getsize(v,nformatstringidx);
    SQInteger allocated = *outlen;
    SQInteger n = 0,i = 0, nparam = nformatstringidx+1, w = 0;
    //while(format[n] != '\0')
    while(n < format_size)
    {
        if(format[n] != '%') {
            ...
            switch(valtype) {
            case 's': bytes = scsprintf(&dest[i],allocated,fmt,ts); i+=bytes;break;
            case 'i': bytes = scsprintf(&dest[i],allocated,fmt,ti); i+=bytes;break;
            case 'f': bytes = scsprintf(&dest[i],allocated,fmt,tf); i+=bytes;break;
            };
			allocated-=bytes;
            nparam ++;
        }
...
{
	SQChar *dest = NULL;
	SQUserPointer destptr;
	SQInteger length;
	if (SQ_SUCCEEDED(sq_getuserpointer(v, 1, &destptr))
		&& SQ_SUCCEEDED(sq_getinteger(v, 2, &length)) )
	{
		dest = (SQChar *)destptr;
		if (SQ_FAILED(_ptr_format(v, 3, &length, dest)))
			return -1;
	}
